...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #8bitTEST\r");
            Send(hSocket, "WHO #8bitTEST\r");
        } else
        {
	    LogPrintf("JOIN #8bit\n");
            // randomly join #8bit00-#8bit05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #8bit%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #8bit%02d\r", channel_number).c_str());
        };
