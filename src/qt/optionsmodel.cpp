...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymize8BitAmount"))
        settings.setValue("nAnonymize8BitAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymize8BitAmount = settings.value("nAnonymize8BitAmount").toLongLong();
    ...
    if (settings.contains("nAnonymize8BitAmount"))
        SoftSetArg("-anonymize8bitamount", settings.value("nAnonymize8BitAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymize8BitAmount:
            nAnonymize8BitAmount = value.toInt();
            settings.setValue("nAnonymize8BitAmount", nAnonymize8BitAmount);
            emit anonymize8BitAmountChanged(nAnonymize8BitAmount);
            break;
        default:
            break;
        }
